//перемешивание базовой сетки
void transposing(int *sudoku){//столбцы становятся строками и наоборот
    for(int i=8; i>0; i--)
        for(int j=0; j<=i; j++)
            swap(*(sudoku+i*9+j), *(sudoku+j*9+i));
}
void swap_strings_small(int *sudoku){//Обмен двух строк в пределах одного района
...
void swap_columns_small(int *sudoku){//Обмен двух столбцов в пределах одного района
...
void swap_strings_area(int *sudoku){//обмен больших строк
    int str1, str2;
...
void swap_columns_area(int *sudoku){//обмен больших столбцов
    int col1, col2;
...
void generator_sudoku(int *sudoku, int &count_zero, int level){
    ...
    do{
        ...
        for(int i=0; i<20; i++){//перемешивание
        x=rand()%5;
        switch(x){
            case 0:
                transposing(sudoku);
        ...
        while(count_prosmotrov!=81){//вычеркивание клеточек
            do{//поиск ненулевой клетки
                x=rand()%9;
                y=rand()%9;
            ...
            *(sudoku+x*9+y)=0;
            ...
            if(temp_level>level){//если полученное судоку имеет более высокий уровень, то отмена
                *(sudoku+x*9+y)=temp;
            }else{
                count_zero++;
